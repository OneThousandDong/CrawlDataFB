...
def initDriver():
    CHROMEDRIVER_PATH = 'D:/facebook/chromedriver.exe'
...
def getContentComment(driver):
    try:
        links = driver.find_elements(By.XPATH,'//*[@id="ufi_pfbid027n5Sh4pcPTbC4EGP4VLuWwK4c97aBjx9A2QfekaiuMeSKBBeF3TQ5tJE4LvJGdqWl"]/div/div[5]')
        print(links)
        # ids = []
        # if (len(links)):
        #     for link in links:
        #         takeLink = link.get_attribute('href').split('ctoken=')[1].split('&')[0]
        #         textCommentElement = driver.find_elements(By.XPATH,('//*[@id="' + takeLink.split('_')[1] + '"]/div/div[1]'))
...
def getAmountOfComments(driver, numberCommentTake):
    try:
        driver.get("https://mbasic.facebook.com/" + "pfbid027n5Sh4pcPTbC4EGP4VLuWwK4c97aBjx9A2QfekaiuMeSKBBeF3TQ5tJE4LvJGdqWl")
        getContentComment(driver)
...
cookie = 'c_user=100005747403745;xs=18%3AuPSDnQTxzjzdUA%3A2%3A1665675887%3A-1%3A6329'
driver = initDriver()
isLive = checkLiveCookie(driver, cookie)
if (isLive):
    # getnumOfPostFanpage(driver, 'cuongphimreview', 500, 'posts.csv')
    # for postId in readData('posts.csv'):
    getAmountOfComments(driver, 1000)
